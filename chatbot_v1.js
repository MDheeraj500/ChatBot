...
async function identify_user(agent)
{
  ...
  if(snap==null){
	  await agent.add("Please enter your registered mobile number.");

  }
  else
  {
  user_name=snap.username;
  await agent.add("Welcome  "+user_name+"  \n\n Please express your issue");}
}
...
function report_issue(agent)
{
 ...
 agent.add("The issue reported is: "+ val +"\nPlease note your ticket number: "+trouble_ticket+"\n\n we have dispactached our agent and the issue will be resolved within the next 24 hours");
}
...
intentMap.set("service_intent", identify_user);//note!!
